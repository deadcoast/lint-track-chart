...
export async function trackLintingProgress() {
  ...
  try {
    ...
    const dirs = [config.reportDir, path.dirname(config.logFile)];
    dirs.forEach(dir => {
    ...
    if (fs.existsSync(jsonPath)) {
      fs.renameSync(jsonPath, path.join(config.reportDir, 'previous.json'));
    }
    fs.writeFileSync(jsonPath, JSON.stringify(jsonResults, null, 2));

    console.log(`\n${getCurrentTheme().successStyle("✓ Analysis complete!")}`);
    console.log(`Results saved to ${getCurrentTheme().infoStyle(config.logFile)}`);
